...
class DbHelper{
  ...
  Future<bool> isEmailAlreadyExists({ required String email})async{
    ...
    Database db=await getDB();
       var mData=await   db.query(TABLE_USERS, where: "$COLUNMN_USER_EMAIL=?" ,whereArgs: [email]);
  ...
  Future<bool>authenticate({required String email, required String password})async{
    Database db=await getDB();
    var mData=await db.query(TABLE_USERS, where: "$COLUNMN_USER_EMAIL=? and $COLUNMN_USER_PASSWORD=?", whereArgs: [email,password]);
